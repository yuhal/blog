...
EOF;
    ...
    protected function addClassMapCode($dir, $namespace, $classMap)
    {
        foreach ($this->createMap($dir, $namespace) as $class => $path) {
            ...
            } elseif ($classMap[$class] !== $pathCode && !preg_match('{/(think-another|fixture|example|stub)s?/}i', strtr($classMap[$class] . ' ' . $path, '\\', '/'))) {
                $this->output->writeln(
    ...
    protected function createMap($path, $namespace = null)
    {
        ...
        foreach ($path as $file) {
            ...
            foreach ($classes as $class) {
                ...
                if (!isset($map[$class])) {
                    $map[$class] = $filePath;
                } elseif ($map[$class] !== $filePath && !preg_match('{/(think-another|fixture|example|stub)s?/}i', strtr($map[$class] . ' ' . $filePath, '\\', '/'))) {
                    $this->output->writeln(
                        '<warning>Warning: Ambiguous class resolution, "' . $class . '"' .
